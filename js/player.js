...
function audioErr() {
    // 没播放过，直接跳过
    if(rem.dislist == 0){
        layer.msg('此歌曲不可播放，请尝试其他音源');
        return false;
    }
    
    if(rem.playlist === undefined) return true;
    
    if(rem.errCount > musicList[1].item.length) { // 连续播放失败的歌曲过多
        layer.msg('播放列表歌曲均已失效，请尝试刷新页面');
        rem.errCount = 0;
        window.location.href=window.location.href;
        return false;   
    }
    if(rem.chongshi == false){
        rem.chongshi = true;
        
        if(musicList[1].item[rem.playid] != undefined && musicList[1].item[rem.playid] != null) {
            if(musicList[1].item[rem.playid].url  != undefined){
                musicList[1].item[rem.playid].url = "";
                playList(rem.playid);
            }
        }
    }else {
...
function audioPlay() {
    ...
    var msg = " 正在为美嘉播放: " + music.name + " - " + music.artist;  // 改变浏览器标题
...
function listClick(no) {
    ...
    if(rem.dislist === 0) {
        ...
        for(var i=0; i<musicList[3].item.length; i++) {
            if(musicList[3].item[i].id == tmpMusic.id && musicList[3].item[i].source == tmpMusic.source) {
                tmpMusic.tmpt = 'a';
            }
        }
...
//        }
    ...
    if(musicList[1].item[id].scsj != undefined && musicList[1].item[id].scsj > 1000){
        var timestamp = Math.round(new Date().getTime()/1000);
        var bfsc = timestamp - musicList[1].item[id].scsj;
        if(bfsc > 1680){
    ...
    if(musicList[1].item[id].url === null || musicList[1].item[id].url === "") {
        var timestamp = Math.round(new Date().getTime()/1000);
        musicList[1].item[id].scsj = timestamp;
...
//    }
    ...
    rem.playmid = music.id;
}
// 音乐进度条拖动回调函数
function mBcallback(newVal) {
    //layer.msg('对不起美嘉，快进的代码还没有写');
