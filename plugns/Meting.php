<?php
function get_mp3url($val, $type){
    //$suppose = array('netease', 'tencent', 'xiami', 'kugou', 'baidu');

//        if($type == 'netease'){
//            //http://m8.music.126.net/20210121020826/0e9defc8d12da9242aa9aa9bd2f719a3/ymusic/030f/0659/0652/4eeba3ec67139de17412186b850c3a70.mp3
//            $tou = get_between($val, '://', '.');
//            $urlInfo = parse_url($val);
//            $url = substr($urlInfo['path'],1);
//            $newUrl = 'http://你的反向代理地址/?tou='.$tou.'&url='.$url;
//            return $newUrl;
//        }
//    
//    if($type == 'kugou'){
//        //http://fs.ios.kugou.com/202101210147/517a8d8a485cc7c4245138dca48353bb/G202/M06/0C/10/Cg4DAF5trh6ALfzXAD1JyzTgRHo016.mp3
//        $tmpval = '你的反向代理地址';
//        $newUrl = str_replace('fs.ios.kugou.com', $tmpval, $val);
//        return $newUrl;
//    }
...
class Meting
{
    ...
    private function netease_url($result)
    {
        ...
        if (isset($data['data'][0]['url'])) {
            $url = array(
                'url'  => get_mp3url($data['data'][0]['url'], 'netease'),
    ...
    private function xiami_url($result)
    {
        ...
        foreach ($data['data']['data']['songs'][0]['listenFiles'] as $vo) {
            if ($type[$vo['quality']] <= $this->temp['br'] && $type[$vo['quality']] > $max) {
                $max = $type[$vo['quality']];
                $url = array(
                    'url'  => get_mp3url($vo['listenFile'], 'xiami'),
                    'size' => $vo['fileSize'],
                    'br'   => $type[$vo['quality']],
                );
            }
        ...
        foreach ($data['data'][0]['relate_goods'] as $vo) {
            if ($vo['info']['bitrate'] <= $this->temp['br'] && $vo['info']['bitrate'] > $max) {
                ...
                if (isset($t['url'])) {
                    $max = $t['bitRate'] / 1000;
                    $url = array(
                        'url'  => get_mp3url(reset($t['url']), 'kugou'),
                        'size' => $t['fileSize'],
                        'br'   => $t['bitRate'] / 1000,
                    );
                }
